...
def color_deuda(row):
    color = ''
    try:
        if row['Estado_Pago'] == 'Pagado': color = 'background-color: #d4edda; color: #155724;'
...
# --- INICIO INTERFAZ ---
st.title("🌱 Agrícola Montserrat - Gestión Financiera Total")
...
# PESTAÑAS
tab1, tab_ana, tab_gastos, tab2, tab3 = st.tabs(["📊 Dashboard Financiero", "📈 Analítica", "💸 Gastos Operativos", "🧮 Nueva Operación", "🚦 Cartera"])

# --- TAB 1: DASHBOARD (EL CEREBRO) ---
with tab1:
    st.markdown("### 💰 Resultado Real del Periodo")
    ...
    k1.metric("Utilidad Operativa (Viajes)", f"${utilidad_bruta:,.0f}", delta="Ingresos Brutos")
    k2.metric("Gastos Fijos/Admin", f"${total_gastos:,.0f}", delta="- Salidas", delta_color="inverse")
    k3.metric("💵 UTILIDAD NETA REAL", f"${utilidad_neta_real:,.0f}", delta="Bolsillo Real")
    k4.metric("📦 Volumen Total", f"{kg_movidos:,.1f} Kg")
    ...
    with g1:
        ...
        st.plotly_chart(px.bar(datos_bal, x='Concepto', y='Monto', color='Concepto', color_discrete_sequence=['blue', 'red', 'green']), use_container_width=True)

    with g2:
        if not df_g_filtrado.empty:
            st.subheader("¿En qué se va la plata? (Gastos)")
            st.plotly_chart(px.pie(df_g_filtrado, values='Monto', names='Concepto', hole=0.4), use_container_width=True)
        else:
            st.info("No hay gastos registrados en este periodo.")

# --- TAB NUEVA: GASTOS OPERATIVOS ---
with tab_gastos:
    st.header("💸 Registro de Gastos Fijos")
    st.caption("Gasolina, nómina, reparaciones, planes de celular, etc.")
    
    # Formulario Gasto
    with st.form("form_gasto", clear_on_submit=True):
        c_g1, c_g2, c_g3 = st.columns(3)
        fecha_g = c_g1.date_input("Fecha Gasto", date.today())
        concepto_g = c_g2.text_input("Concepto", placeholder="Ej: Gasolina Moto")
    ...
    if not df_g_filtrado.empty:
        ...
        # Borrar Gasto
        with st.expander("🗑️ Borrar un gasto incorrecto"):
            id_borrar = st.number_input("ID del gasto a borrar", min_value=1, step=1)
    ...
    if not df_v_filtrado.empty:
        ...
        with c_graf:
            if not df_plaza.empty:
                ...
                fig = px.line(df_line, x='Fecha', y=['Precio_Compra', 'Precio_Plaza'], markers=True, title="Compra vs Plaza")
        ...
        with c_info:
            if not df_plaza.empty:
                ...
                if diff > 0: st.success(f"Ahorro: ${diff:,.0f}/kg")
                else: st.error(f"Sobreprecio: ${abs(diff):,.0f}/kg")
...
with tab2:
    ...
    with st.form("registro_viaje", clear_on_submit=True):
        ...
        n_prov = r3.text_input("Otro prov.", placeholder="Escribe si no está")

        l_cli = obtener_opciones(df_ventas, 'Cliente', ["Calima", "Fogón", "HEFE"])
        s_cli = r4.selectbox("Cliente", l_cli)
        n_cli = r4.text_input("Otro cli.", placeholder="Escribe si no está")
        ...
        st.metric("Utilidad Est.", f"${util_est:,.0f}")
...
with tab3:
    ...
    if not df_v_filtrado.empty:
        ...
        if event.selection.rows:
            ...
            st.divider()
            with st.form("edit"):
                st.write(f"Editando: {row['Cliente']}")
