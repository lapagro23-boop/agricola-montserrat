...
# Recuperamos las credenciales desde los 'secrets' de Streamlit (ver Paso 3)
# Si lo corres local, puedes poner tus claves aquí directamente (pero no lo subas a GitHub así)
try:
...
BUCKET_FACTURAS = "facturas" # Asegúrate de crear este bucket en Supabase Storage

# --- 2. FUNCIONES DE BASE DE DATOS (NUBE) ---
def cargar_datos():
    # Traemos todos los datos de la tabla 'ventas_2026'
    response = supabase.table("ventas_2026").select("*").execute()
    df = pd.DataFrame(response.data)
    
    if not df.empty:
        # Renombramos columnas de minúsculas (Supabase) a las que usa tu código original
        mapa_cols = {
...
def subir_archivo(archivo, nombre_base):
    """Sube el archivo al Storage de Supabase y devuelve la URL pública"""
    if archivo:
        try:
            nombre_archivo = f"{nombre_base}_{archivo.name}"
            # Convertimos a bytes para subir
...
with tab1:
    if not df.empty:
        ...
        st.info("No hay datos para mostrar en este rango de fechas.")

# --- TAB 2: CALCULADORA (SUBIDA A NUBE) ---
with tab2:
            ...
            if prov and cli:
                with st.spinner("Subiendo datos a Supabase..."):
                    ...
                    datos_nuevos = {
                        ...
                        "proveedor": prov,
                        "cliente": cli,
                        "fec_doc_url": url_compra,
                        "fev_doc_url": url_venta,
...
with tab3:
    ...
    if not df.empty:
        # Mostramos enlaces clickeables si existen facturas
        st.dataframe(
